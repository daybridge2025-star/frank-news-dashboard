...
5. **Finnhub 경제 캘린더(`/calendar/economic`) — 무료 플랜 403 실측 확정(2026-07-12)**: 정기
   ...
   기존 403 처리, Finnhub 문서의 "Premium" 배지. **대안 결정 대기**(사용자 판단): ①FOMC 연 8회
   고정 일정 JSON(완전 무료·수동 연 1~2회 갱신) ②FRED 릴리스 캘린더 재사용(기존 FRED_API_KEY,
   CPI·고용·GDP 발표일 커버, 컨센서스 없음) ③Finnhub 유료 전환. 어느 쪽이든 수집기만 교체하면
   되고 HTML 섹션·렌더러·마커는 그대로 재사용된다.
