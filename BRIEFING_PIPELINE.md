...
2. **컨테이너 시계 오차**: 세션 컨테이너의 시계가 실제보다 수 시간 빠르게 설정된 사례 확인
   (커밋이 미래 시각으로 기록되고, 아직 오지 않은 "미국장 마감 에디션"으로 자기 판단함).
   **에디션(아침/저녁) 판단과 날짜 라벨은 컨테이너 시계를 믿지 말고, `data/krx_snapshot_latest.json`의
   `bas_dd`·`fetched_at`과 최신 커밋 시각을 기준으로 삼을 것.** 미국 장중 데이터(예: ADR 데뷔가)를
   쓰기 전에 그 장이 실제로 열렸는지 스냅샷 기준일과 대조해 확인.
3. **HTML 커밋 주체**: 세션이 JSON 갱신 후 직접 `build_briefing.py`를 돌려 HTML까지 커밋해도
   되고(렌더러가 멱등이라 Action과 충돌하지 않음), 다음 Action 실행에 맡겨도 된다. 단 커밋은
   반드시 자기 명의로 할 것 — `github-actions[bot]` 명의를 빌려 쓰면 장애 추적이 어려워진다.
