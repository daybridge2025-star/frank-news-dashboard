...
7. **포트폴리오 탭(2026-07-17 신설)**: 사용자 요청으로 `#tab-pf`(하단 탭바 6번째 "포트폴리오")를
   추가 — 실계좌 2곳 스크린샷 기준 보유 내역 + 전략 A/B/C 문서와의 정합성 진단. **데이터 출처가
   자동 수집이 아니라 사용자 제공 스크린샷**이라는 점이 다른 영역과 다르다: 수량·평단·평가액·예수금은
   사용자가 새 내역을 공유하기 전엔 절대 갱신·추정하지 말 것(현재가·손익이 낡아 보여도 그대로 두고
   섹션 상단의 "기준: YYYY-MM-DD" 라벨만 신뢰). 매 실행 점검할 것은 두 가지 — ①진단 문단이 참조하는
   다른 탭의 상태(트리거 status, 스탠스, 시나리오 확률)가 바뀌었으면 진단 서술만 맞춰 갱신,
   ②날짜 박힌 이벤트 문구(예: "7/22 테슬라 실적 예정")의 소멸 여부. 계좌번호 등 식별 정보는
   HTML에 넣지 않는다(현재 계좌①/계좌②로 익명화).
