...
data/econ_calendar_latest.json┘  (이번 주 경제 캘린더 — Action 자동: Finnhub, 플랜 제한 가능성 있음)
```
...
| 이번 주 주요 경제 캘린더(미국·한국 고영향) | `data/econ_calendar_latest.json` | `MKT` | GitHub Action | 자동(Finnhub) — **플랜 제한으로 항상 비어 있을 수 있음, 아래 노트 참고** |
...
5. **Finnhub 경제 캘린더(`/calendar/economic`) 플랜 제한 가능성(2026-07-12)**: 이 저장소의
   `app.py`(Streamlit 대시보드)에 이미 이 엔드포인트를 호출하는 코드가 있고, HTTP 403을 명시적으로
   구분해 "Finnhub 무료 플랜에서 경제캘린더 API가 제한됩니다" 메시지를 띄우는 처리가 존재한다 —
   과거에 실제로 403을 받은 이력이 있다는 뜻으로 읽힌다. Finnhub 공식 문서(`/docs/api/economic-calendar`)
   에도 "Premium" 배지가 붙어 있다. `fetch_econ_calendar.py`는 이 가능성을 감안해 401/403/기타 실패를
   명확히 구분된 로그로 남기도록 짰다 — Action 실행 로그의 `[EconCal]` 줄을 보면 실제 이 세션 키가
   막혀 있는지 바로 확인 가능. 막혀 있으면 `econ_calendar_latest.json`의 `status`가 `"403"`으로
   저장되고, HTML의 캘린더 섹션은 "미확보/집계중" 표기를 그대로 유지한다(지어내지 않음). 실측 결과에
   따라 대안(FOMC 날짜 소규모 고정 JSON, FRED 릴리스 캘린더 재사용, 유료 플랜 전환 등)을 사용자와
   논의할 것.
