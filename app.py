...
def fetch_hyperscaler_data():
    """MSFT·GOOGL·AMZN·META 연간 재무 취득 → 합산 DataFrame 반환 (2020~현재)"""
    ...
    for ticker in HYPSCALER_TICKERS:
        try:
            t   = yf.Ticker(ticker)
            inc = t.income_stmt    # 연간 데이터 (2020~현재, 최대 5년)
            cf  = t.cash_flow      # 연간 현금흐름
            for r in NI_ROWS:
                if r in inc.index:
    ...
    ni_s  = _agg(ni_map)
    ocf_s = _agg(ocf_map)
    cap_s = _agg(cap_map)    # 음수 (지출)
    fcf_s = ocf_s + cap_s    # FCF = OCF + Capex(음수)
